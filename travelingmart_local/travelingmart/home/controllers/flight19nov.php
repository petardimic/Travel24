                ...
                $Fare_MasterPricerTravelBoardSearch = '<?xml version="1.0" encoding="utf-8"?>
                                            <soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
                                             <soapenv:Header>
                                              ...
                                              <soapenv:Body>
												<Fare_MetaPricerTravelboardSearch xmlns="http://xml.amadeus.com/FMPCAQ_08_2_1A">
														<numberOfUnit>
															<unitNumberDetail>
                ...
                $Fare_MasterPricerTravelBoardSearch = '<?xml version="1.0" encoding="utf-8"?>
														<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
														 <soapenv:Header>
														  ...
														  <soapenv:Body>
															<Fare_MetaPricerCalendar xmlns="http://xml.amadeus.com/FMPCAQ_08_2_1A">
																<numberOfUnit>
																	<unitNumberDetail>
																		<numberOfUnits>' . $Pcount . '</numberOfUnits>
																		<typeOfUnit>PX</typeOfUnit>
...
//echo '<prre>';print_r($testing[$i]['duration_final_layover'][$j]);
      ...
      // echo 'fgfgfdg'.$calanderData;exit;
		...
		// echo $calanderData;
		//echo $this->load->view('search_result', $data);
    ...
    public function fetch_flight_search_result_Round_Trip($data, $rand_id) 
    {
		...
		if(!empty($_SESSION['lang']))
		{
			if($_SESSION['lang']=='en')
			{
				$flight_search_result = $this->load->view('flight/search_result_ajax_roundtrip', $data, true);
			}
			else
			{
				$flight_search_result = $this->load->view('flight/danish/search_result_ajax_roundtrip', $data, true);
			}
		}
		else
		{
	...
	function flight_detail_fare($id, $rand_id) 
	{
		...
		if(true)
		{
			...
			$error2 = curl_getinfo($ch2, CURLINFO_HTTP_CODE);
			// curl_close($ch2);			
			// $Fare_CheckRules_result = $this->xml2array($Fare_CheckRules_data);
			// echo '<pre/>';print_r($Fare_CheckRules_result);exit;
			/*if (!isset($Fare_CheckRules_result['soap:Envelope']['soap:Body']['Fare_CheckRulesReply']['errorMessage'])) 
			{
				if (isset($Fare_CheckRules_result['soap:Envelope']['soap:Body']['Fare_CheckRulesReply']))
					$Fare_CheckRules_data = $Fare_CheckRules_result['soap:Envelope']['soap:Body']['Fare_CheckRulesReply'];
				else
					$Fare_CheckRules_data = "";
			}
			else
			{
				$Fare_CheckRules_data = "";
			...
			if($Fare_CheckRules_data != "")
			{
				//$slno =1; 
				$counter_tariffInfo = count($Fare_CheckRules_data['tariffInfo']);
				for($loop =0 ;$loop< $counter_tariffInfo; $loop++)	
				{
					$tariffInfo = $Fare_CheckRules_data['tariffInfo'][$loop]['fareRuleText'];
					//echo '<pre/>';print_r($tariffInfo);die;
					$temp_counter = count($tariffInfo);
					for($loops =0 ;$loops< $temp_counter; $loops++)	
					{
						if(!empty($tariffInfo[$loops]['freeText']))
						{
							$display_rules1 .= '<tr>
											<td>'.$slno .'</td>
											<td>&nbsp;</td>
											<td >'.trim($tariffInfo[$loops]['freeText']).'</td>
										</tr>';	
						}
						$slno++;
					}
			...
			// echo '<pre/>sdsa';print_r($tariffInfo);exit;
			
			echo $display_rules; exit;
		    ...
		    public function fetch_flight_search_result2($session_id,$norway_session,$rand_id)
    {
       ...
       // $airline_filter_result=$this->load->view('flight/search-result',$airline_data,true);
        //$stops_filter_result=$this->load->view('flight/search-result',$stops_data,true);
        //$matrix_search_result= $this->load->view('flight/search-result',$matrixData,true);
       /* echo json_encode(array(
            'flight_search_result' => $flight_search_result,
