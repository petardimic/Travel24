...
class Flights extends MX_Controller
{
    ...
    // {
             ...
             if ((isset($_POST['from_city1'])) && (isset($_POST['to_city1'])))
             {
       ...
       //     }
                    ...
                    $_SESSION['fromcityval'] = $_POST['from_city1'];
                    $_SESSION['tocityval'] = $_POST['to_city1'];
                    ...
                    $this->session->set_userdata(array('fromcityval'=>$_POST['from_city1'],'tocityval'=>$_POST['to_city1'],'sd'=>$_POST['sd'],'ed'=>$_POST['ed'],'adults'=>$adult_count,'childs'=>$child_count,'infants'=>$infant_count,'journey_types'=>$_POST['journey_type'],'cabin_selected'=>$_POST['cabin']));
    ...
    function cancel_policy($id,$rand_id)
    {
		
		$flightDetailsQuery=$this->db->query($sql="select * from flight_search_result where id='".$id."'");
		if($flightDetailsQuery->num_rows() > 0)
		{
			$flightDetails=$flightDetailsQuery->row_array();
		}
		else $flightDetails='';
		
		if($flightDetails!='')
		{
			if(strpos($flightDetails->fareType,'<br>'))
			{
				$ex=explode('<br>',$flightDetails['fareType']);
				for($j=0;$j<count($ex);$j++)
				{
					$rules='<ruleSectionId>'.$ex[$j].'</ruleSectionId>';
				}
			}
			else
			{
				$rules='<ruleSectionId>'.$flightDetails['fareType'].'</ruleSectionId>';
			}
	/*	$session_flag = 'true';
		...
		$Passenger_group_ADT='';
			...
			if(strpos($flightDetails['dlocation'],'<br>'))
			{
				$dlocationArray=explode('<br>',$flightDetails['dlocation']);
				for($l= 0;$l < (count($dlocationArray)); $l++)
				{ 
					$dlocationArray=explode('<br>',$flightDetails['dlocation']);
					...
					$BookingClassArray=explode('<br>',$flightDetails['BookingClass']);
					
					$origin=$dlocation[0];
					$destination=$alocation[$l];
					$segment_details.= '<segmentGroup>
											<segmentInformation>
											<flightDate>
												<departureDate>'.$dateOfDeparture[$l].'</departureDate>
											</flightDate>
											<boardPointDetails>
												<trueLocationId>'.$dlocation[$l].'</trueLocationId>
											</boardPointDetails>
											<offpointDetails>
												<trueLocationId>'.$alocation[$l].'</trueLocationId>
											</offpointDetails>
											<companyDetails>
												<marketingCompany>'.$cicode[$l].'</marketingCompany>
											</companyDetails>
											<flightIdentification>
												<flightNumber>'.$fnumber[$l].'</flightNumber>
												<bookingClass>'.$BookingClass[$l].'</bookingClass>
											</flightIdentification>
										</segmentInformation>
										</segmentGroup>';
				}
		...
		$SessionId=$_SESSION['amed_session'];
		$SequenceNumber=$_SESSION['amed_sequence'];
		$farexml = '<?xml version="1.0" encoding="utf-8"?>
                                                <soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
                                                        <soapenv:Header>
																  <SessionId>' . $SessionId . '</SessionId>
																  <SequenceNumber>' . ($SequenceNumber+1) . '</SequenceNumber>
                                                        </soapenv:Header>
															<Fare_InformativePricingWithoutPNR xmlns="http://xml.amadeus.com/TIPNRQ_08_1_1A">
																<messageDetails>
		...
		$URL2 = "https://test.webservices.amadeus.com";
			// $URL2 = "https://production.webservices.amadeus.com";
            $soapAction = "http://webservices.amadeus.com/1ASIWATOAKB/TIPNRQ_08_1_1A";
           // echo $farexml;//die;
            $ch2 = curl_init();
			curl_setopt($ch2, CURLOPT_URL, $URL2);
			curl_setopt($ch2, CURLOPT_TIMEOUT, 180);
			...
			curl_setopt($ch2, CURLOPT_RETURNTRANSFER, 1); 
			curl_setopt($ch2, CURLOPT_POST, 1);
			curl_setopt($ch2, CURLOPT_POSTFIELDS, $farexml); 
		...
		$xml = '<?xml version="1.0" encoding="utf-8"?>
                                                <soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
													 ...
													 echo $xml; //die;
							$URL2 = "https://production.webservices.amadeus.com";
                            $soapAction = "http://webservices.amadeus.com/1ASIWATOAKB/FARQNQ_07_1_1A";
                            $ch2 = curl_init();
                            curl_setopt($ch2, CURLOPT_URL, $URL2);
                            curl_setopt($ch2, CURLOPT_TIMEOUT, 180);
                            curl_setopt($ch2, CURLOPT_HEADER, 0);
    ...
    function get_flight_availabilty($rand_id)
    {
        ...
        else 
        {
         ...
         $depCode=explode(',',$fromcityval);
         $depCountry=$this->Flights_Model->get_airportname($depCode[1]);
         if($depCountry->country=='Canada')
         {
             $officeId='<sourceOffice>YTOC42195</sourceOffice>';
         }
         else
         {
             $officeId='<sourceOffice>LGA1S211T</sourceOffice>';
         }
        ...
        if ($session_flag == "true") 
        {
         ...
         //echo $SessionId.'<<<>>>'.$SequenceNumber;die;
	...
	//echo $cin." ";
         ...
         */
			...
			else
			{
                ...
                //echo $SessionId.'<<<>>>'.$SequenceNumber;die;
		$_SESSION['amed_session']=$SessionId;
		$_SESSION['amed_sequence']=$SequenceNumber;
        $dataforgetavail = "";
        ...
        if ($_SESSION['journey_type'] != "MultiCity") {
            ...
            //echo '<pre/>'.$Fare_MasterPricerTravelBoardSearch;die;
            //$URL2 = "https://production.webservices.amadeus.com";
				...
				foreach ($flight_result as $flight_result) 
				{
					...
					if ($count_code <= 1) 
					{
						...
						if ($name != '') 
						{
							...
							///echo '<pre />';print_r($flight_result['paxFareProduct']);die;
							...
							//echo '<pre />fhgfghgfhggf';print_r($flight_result['paxFareProduct']);die;
							if (isset($flight_result['paxFareProduct'][0]['rbd'])) 
							{
								...
								$testing1[$i]['fareType'] = $flight_result['paxFareProduct'][0]['fareType'];
							} 
							else 
							{
							...
							if(isset($flight_result['paxFareProduct'][0]) && isset($flight_result['paxFareProduct'][1]) && isset($flight_result['paxFareProduct'][2]))
							{
								$testing1[$i]['Adult_FareAmount']=$flight_result['paxFareProduct'][0]['totalFareAmount'];
								$testing1[$i]['Adult_TaxAmount']=$flight_result['paxFareProduct'][0]['totalTaxAmount'];
								$testing1[$i]['Child_FareAmount']=$flight_result['paxFareProduct'][1]['totalFareAmount'];
								$testing1[$i]['Child_TaxAmount']=$flight_result['paxFareProduct'][1]['totalTaxAmount'];
								$testing1[$i]['Infant_FareAmount']=$flight_result['paxFareProduct'][2]['totalFareAmount'];
								$testing1[$i]['Infant_TaxAmount']=$flight_result['paxFareProduct'][2]['totalTaxAmount'];
							}
							else if(isset($flight_result['paxFareProduct'][0]) && isset($flight_result['paxFareProduct'][1]) && !isset($flight_result['paxFareProduct'][2]))
							{
								$testing1[$i]['Adult_FareAmount']=$flight_result['paxFareProduct'][0]['totalFareAmount'];
								$testing1[$i]['Adult_TaxAmount']=$flight_result['paxFareProduct'][0]['totalTaxAmount'];
								$testing1[$i]['Child_FareAmount']=$flight_result['paxFareProduct'][1]['totalFareAmount'];
								$testing1[$i]['Child_TaxAmount']=$flight_result['paxFareProduct'][1]['totalTaxAmount'];
								$testing1[$i]['Infant_FareAmount']=0;
								$testing1[$i]['Infant_TaxAmount']=0;
							}
							else
							{
								$testing1[$i]['Adult_FareAmount']=$flight_result['paxFareProduct']['totalFareAmount'];
								$testing1[$i]['Adult_TaxAmount']=$flight_result['paxFareProduct']['totalTaxAmount'];
								$testing1[$i]['Child_FareAmount']=0;
								$testing1[$i]['Child_TaxAmount']=0;
								$testing1[$i]['Infant_FareAmount']=0;
								$testing1[$i]['Infant_TaxAmount']=0;
							}
							//else if()
						...
						for ($j = 0; $j < ($count_code); $j++) 
						{
							...
							if ($name != '') 
							{
								...
								if (isset($flight_result['paxFareProduct'][0]['rbd'][$j])) 
								{
									...
									$testing1[$i]['fareType'][$j] = $flight_result['paxFareProduct'][0]['fareType'][$j];
								} 
								else 
								{
									$testing1[$i]['BookingClass'][$j] = $flight_result['paxFareProduct']['rbd'][$j];
									$testing1[$i]['cabin'][$j] = $flight_result['paxFareProduct']['cabin'][$j];
									$testing1[$i]['fareType'][$j] = $flight_result['paxFareProduct']['fareType'][$j];
								}
								...
								{
									$testing1[$i]['Adult_FareAmount'][$j]=$flight_result['paxFareProduct']['totalFareAmount'];
									$testing1[$i]['Adult_TaxAmount'][$j]=$flight_result['paxFareProduct']['totalTaxAmount'];
									$testing1[$i]['Child_FareAmount'][$j]=0;
									$testing1[$i]['Child_TaxAmount'][$j]=0;
									$testing1[$i]['Infant_FareAmount'][$j]=0;
									$testing1[$i]['Infant_TaxAmount'][$j]=0;
								}
							}
						}
						$i++;
					...
					if($testing1[$jj]['stops'] > 0)
					{
						...
						//echo '<pre />';print_r($testing1);die;
						if(is_array($testing1[$jj]['Adult_FareAmount']))
						{
							$Adult_FareAmount=$testing1[$jj]['Adult_FareAmount'][0];
							$Adult_TaxAmount=$testing1[$jj]['Adult_TaxAmount'][0];
							$Child_FareAmount=$testing1[$jj]['Child_FareAmount'][0];
							$Child_TaxAmount=$testing1[$jj]['Child_TaxAmount'][0];
							$Infant_FareAmount=$testing1[$jj]['Infant_FareAmount'][0];
							$Infant_TaxAmount=$testing1[$jj]['Infant_TaxAmount'][0];
						}
						else
						{
							$Adult_FareAmount=$testing1[$jj]['Adult_FareAmount'];
							$Adult_TaxAmount=$testing1[$jj]['Adult_TaxAmount'];
							$Child_FareAmount=$testing1[$jj]['Child_FareAmount'];
							$Child_TaxAmount=$testing1[$jj]['Child_TaxAmount'];
							$Infant_FareAmount=$testing1[$jj]['Infant_FareAmount'];
							$Infant_TaxAmount=$testing1[$jj]['Infant_TaxAmount'];
						}
						$insert_array[]=array(
						...
						'Adult_FareAmount'=>$Adult_FareAmount,'Adult_TaxAmount'=>$Adult_TaxAmount,'Child_FareAmount'=>$Child_FareAmount,'Child_TaxAmount'=>$Child_TaxAmount,'Infant_FareAmount'=>$Infant_FareAmount,'Infant_TaxAmount'=>$Infant_TaxAmount);
					}
					else
					{
						...
						'Adult_FareAmount'=>$testing1[$jj]['Adult_FareAmount'],'Adult_TaxAmount'=>$testing1[$jj]['Adult_TaxAmount'],'Child_FareAmount'=>$testing1[$jj]['Child_FareAmount'],'Child_TaxAmount'=>$testing1[$jj]['Child_TaxAmount'],'Infant_FareAmount'=>$testing1[$jj]['Infant_FareAmount'],'Infant_TaxAmount'=>$testing1[$jj]['Infant_TaxAmount']);
					}
...
//                                        $testing[$rt]['paxFareProduct']['fareType'] = $s['paxFareProduct']['fareDetails']['groupOfFares']['productInformation']['fareProductDetail']['fareType'];
													...
													if($flight_result1['MultiTicket']=="Yes")
													{
														...
														if (isset($flight_result1['Recomm'][0]['paxFareProduct'][0])) 
														{
															if(isset($flight_result1['Recomm'][0]['paxFareProduct'][0]['fareDetails'][0]))
															{
																...
																$testing1['oneway'][$i]['fareType'] = $flight_result1['Recomm'][0]['paxFareProduct'][0]['fareDetails'][0]['fareType'];
															}
															else
															{
																...
																$testing1['oneway'][$i]['fareType'] = $flight_result1['Recomm'][0]['paxFareProduct'][0]['fareDetails']['fareType'];
														...
														else 
														{
															if(isset($flight_result1['Recomm'][0]['paxFareProduct']['fareDetails'][0]))
															{
																...
																$testing1['oneway'][$i]['fareType'] = $flight_result1['Recomm'][0]['paxFareProduct']['fareDetails'][0]['fareType'];
															}
															else
															{
																...
																$testing1['oneway'][$i]['fareType'] = $flight_result1['Recomm'][0]['paxFareProduct']['fareDetails']['fareType'];
															}
														}
													}
													else
													{
														...
														if (isset($flight_result1['Recomm']['paxFareProduct'][0])) 
														{
															if(isset($flight_result1['Recomm']['paxFareProduct'][0]['fareDetails'][0]))
															{
																...
																$testing1['oneway'][$i]['fareType'] = $flight_result1['Recomm']['paxFareProduct'][0]['fareDetails'][0]['fareType'];
															}
															else
															{
																...
																$testing1['oneway'][$i]['fareType'] = $flight_result1['Recomm']['paxFareProduct'][0]['fareDetails']['fareType'];
															}
														} 
														else 
														{
															if(isset($flight_result1['Recomm']['paxFareProduct']['fareDetails'][0]))
															{
																...
																$testing1['oneway'][$i]['fareType'] = $flight_result1['Recomm']['paxFareProduct']['fareDetails'][0]['fareType'];
															}
															else
															{
																...
																$testing1['oneway'][$i]['fareType'] = $flight_result1['Recomm']['paxFareProduct']['fareDetails']['fareType'];
															}
														}
													}
													   ...
													   if($flight_result1['MultiTicket']!="Yes")
													   {
															...
															if (isset($flight_result1['Recomm']['paxFareProduct'][0])) 
															{
																if(isset($flight_result1['Recomm']['paxFareProduct'][0]['fareDetails'][0]))
																{
																	...
																	$testing1['oneway'][$i]['fareType'][$j] = $flight_result1['Recomm']['paxFareProduct'][0]['fareDetails'][0]['fareType'][$j];
																}
																else
																{
																	...
																	$testing1['oneway'][$i]['fareType'][$j] = $flight_result1['Recomm']['paxFareProduct'][0]['fareDetails']['fareType'][$j];								
																}
															} 
															else 
															{
																if(isset($flight_result1['Recomm']['paxFareProduct']['fareDetails'][0]))
																{
																	...
																	$testing1['oneway'][$i]['fareType'] = $flight_result1['Recomm']['paxFareProduct']['fareDetails'][0]['fareType'][$j];
																}
																else
																{
																	...
																	if(isset($flight_result1['Recomm']['paxFareProduct']['fareDetails']['fareType'][$j]))
																		$testing1['oneway'][$i]['fareType'] = $flight_result1['Recomm']['paxFareProduct']['fareDetails']['fareType'][$j];
																	else
																		$testing1['oneway'][$i]['fareType'] = $flight_result1['Recomm']['paxFareProduct']['fareDetails']['fareType'];
																}
															}
														...
														else
														{
															...
															if (isset($flight_result1['Recomm'][0]['paxFareProduct'][0])) 
															{
																if(isset($flight_result1['Recomm'][0]['paxFareProduct'][0]['fareDetails'][0]))
																{
																	...
																	$testing1['oneway'][$i]['fareType'][$j] = $flight_result1['Recomm'][0]['paxFareProduct'][0]['fareDetails'][0]['fareType'][$j];
																}
																else
																{
																	...
																	$testing1['oneway'][$i]['fareType'][$j] = $flight_result1['Recomm'][0]['paxFareProduct'][0]['fareDetails']['fareType'][$j];								
																}
															} 
															else 
															{
																if(isset($flight_result1['Recomm'][0]['paxFareProduct']['fareDetails'][0]))
																{
																	...
																	$testing1['oneway'][$i]['fareType'] = $flight_result1['Recomm'][0]['paxFareProduct']['fareDetails'][0]['fareType'][$j];
																}
																else
																{
																	...
																	if(isset($flight_result1['Recomm'][0]['paxFareProduct']['fareDetails']['cabin'][$j]))
																		...
																		$testing1['oneway'][$i]['cabin'] = $flight_result1['Recomm'][0]['paxFareProduct']['fareDetails']['cabin'];
																	if(isset($flight_result1['Recomm'][0]['paxFareProduct']['fareDetails']['fareType'][$j]))
																		$testing1['oneway'][$i]['fareType'] = $flight_result1['Recomm'][0]['paxFareProduct']['fareDetails']['fareType'][$j];
																	else
																		$testing1['oneway'][$i]['fareType'] = $flight_result1['Recomm'][0]['paxFareProduct']['fareDetails']['fareType'];
																}
															}
													...
													if($flight_result2['MultiTicket']=="Yes")
													{
														...
														if (isset($flight_result1['Recomm'][1]['paxFareProduct'][0])) {
															if(isset($flight_result2['Recomm'][1]['paxFareProduct'][0]['fareDetails'][1]))
															{
																...
																$testing1['Return'][$i]['fareType'] = $flight_result2['Recomm'][1]['paxFareProduct'][0]['fareDetails'][1]['fareType'];
															}
															else
															{
																...
																$testing1['Return'][$i]['fareType'] = $flight_result2['Recomm'][1]['paxFareProduct'][0]['fareDetails']['fareType'];
															}
														} else {
															if(isset($flight_result2['Recomm'][1]['paxFareProduct']['fareDetails'][1]))
															{
																...
																$testing1['Return'][$i]['fareType'] = $flight_result2['Recomm'][1]['paxFareProduct']['fareDetails'][1]['fareType'];
															}
															else
															{
																...
																$testing1['Return'][$i]['fareType'] = $flight_result2['Recomm'][1]['paxFareProduct']['fareDetails']['fareType'];
															}
														}
													}
													else
													{
														...
														if (isset($flight_result1['Recomm']['paxFareProduct'][0])) {
															if(isset($flight_result2['Recomm']['paxFareProduct'][0]['fareDetails'][1]))
															{
																...
																$testing1['Return'][$i]['fareType'] = $flight_result2['Recomm']['paxFareProduct'][0]['fareDetails'][1]['fareType'];
															}
															else
															{
																...
																$testing1['Return'][$i]['fareType'] = $flight_result2['Recomm']['paxFareProduct'][0]['fareDetails']['fareType'];
															}
														} else {
															if(isset($flight_result2['Recomm']['paxFareProduct']['fareDetails'][1]))
															{
																...
																$testing1['Return'][$i]['fareType'] = $flight_result2['Recomm']['paxFareProduct']['fareDetails'][1]['fareType'];
															}
															else
															{
																...
																$testing1['Return'][$i]['fareType'] = $flight_result2['Recomm']['paxFareProduct']['fareDetails']['fareType'];
															}
														}
													}
														...
														if($flight_result2['MultiTicket']=="Yes")
														{
															...
															if (isset($flight_result1['Recomm'][1]['paxFareProduct'][0])) 
															{
																if(isset($flight_result2['Recomm'][1]['paxFareProduct'][0]['fareDetails'][1]))
																{
																	...
																	$testing1['Return'][$i]['fareType'][$j] = $flight_result2['Recomm'][1]['paxFareProduct'][0]['fareDetails'][1]['fareType'][$j];
																}
																else
																{
																	...
																	$testing1['Return'][$i]['fareType'][$j] = $flight_result2['Recomm'][1]['paxFareProduct'][0]['fareDetails']['fareType'][$j];
																}
															} 
															else 
															{
																if(isset($flight_result2['Recomm'][1]['paxFareProduct']['fareDetails'][1]))
																{
																	...
																	$testing1['Return'][$i]['fareType'] = $flight_result2['Recomm'][1]['paxFareProduct']['fareDetails'][1]['fareType'][$j];
																}
																else
																{
																	...
																	if(isset($flight_result2['Recomm'][1]['paxFareProduct']['fareDetails']['cabin'][$j]))
																		...
																		$testing1['Return'][$i]['cabin'] = $flight_result2['Recomm'][1]['paxFareProduct']['fareDetails']['cabin'];
																	if(isset($flight_result2['Recomm'][1]['paxFareProduct']['fareDetails']['fareType'][$j]))
																		$testing1['Return'][$i]['fareType'] = $flight_result2['Recomm'][1]['paxFareProduct']['fareDetails']['fareType'][$j];
																	else
																		$testing1['Return'][$i]['fareType'] = $flight_result2['Recomm'][1]['paxFareProduct']['fareDetails']['fareType'];
																}
															}
														...
														else
														{
															...
															if (isset($flight_result1['Recomm']['paxFareProduct'][0])) {
																if(isset($flight_result2['Recomm']['paxFareProduct'][0]['fareDetails'][1]))
																{
																	...
																	$testing1['Return'][$i]['fareType'][$j] = $flight_result2['Recomm']['paxFareProduct'][0]['fareDetails'][1]['fareType'][$j];
																}
																else
																{
																	...
																	$testing1['Return'][$i]['fareType'][$j] = $flight_result2['Recomm']['paxFareProduct'][0]['fareDetails']['fareType'][$j];
																}
															} 
															else 
															{
																if(isset($flight_result2['Recomm']['paxFareProduct']['fareDetails'][1]))
																{
																	...
																	$testing1['Return'][$i]['fareType'] = $flight_result2['Recomm']['paxFareProduct']['fareDetails'][1]['fareType'][$j];
																}
																else
																{
																	...
																	if(isset($flight_result2['Recomm']['paxFareProduct']['fareDetails']['cabin'][$j]))
																		...
																		$testing1['Return'][$i]['cabin'] = $flight_result2['Recomm']['paxFareProduct']['fareDetails']['cabin'];
																	if(isset($flight_result2['Recomm']['paxFareProduct']['fareDetails']['fareType'][$j]))
																		$testing1['Return'][$i]['fareType'] = $flight_result2['Recomm']['paxFareProduct']['fareDetails']['fareType'][$j];
																	else
																		$testing1['Return'][$i]['fareType'] = $flight_result2['Recomm']['paxFareProduct']['fareDetails']['fareType'];
																}
															}
           ...
           // echo '<pre />dsfdsfsdf';print_r($testing1);
            ...
            for($r=0;$r < $countOneway;$r++)
            {
					if($testing1['oneway'][$r]['stops']>0)
					{
						...
						if(is_array($fareType))
						  {
						   $fareType=implode('<br>',$fareType);
						  }
						  else { $fareType=$fareType; }
						$insert_array[]=array(
...
//	}
    ...
    public function fetch_flight_search_resultcache($fromcityval_for_cache,$tocityval_for_cache,$sd_cache,$ed_cache,$adults_cache,$childs_cache,$infants_cache,$journey_types_cache,$cabin_selected_cache)
    {
        ...
        $flight_resultmatrix = $this->Flights_Model->getFlightSearchResultmatrix($fromcityval_for_cache,$tocityval_for_cache,$sd_cache,$ed_cache,$adults_cache,$childs_cache,$infants_cache,$journey_types_cache,$cabin_selected_cache);
        $data['flight_result']=$flightResult['search_result'];
        $data['flight_resultmatrix'] =$flight_resultmatrix['search_result'];
      //echo '<pre />dfsfsdf';print_r($flight_resultmatrix);die;
        $data['session_id']=$_SESSION['session_id'];
        $data['akbar_session']=$_SESSION['akbar_session'];
        $flight_result = $data['flight_result'];
    ...
    public function fetch_flight_search_result($session_id,$akbar_session,$rand_id)
    {
        ...
        $flight_resultmatrix = $this->Flights_Model->getFlightSearchResultmatrix_normal($session_id,$akbar_session);
        $data['flight_result']=$flightResult['search_result'];
        $data['session_id']=$session_id;
        $data['akbar_session']=$akbar_session;
        $data['flight_resultmatrix'] =$flight_resultmatrix['search_result'];
        $flight_result = $data['flight_result'];
        $_SESSION[$rand_id]['flight_result1'] = $flight_result;
        if($data['flight_result']!='')
    ...
    public function fetch_flight_search_result_Round_Trip($session_id,$akbar_session,$rand_id)
    {
        ...
        $flight_resultmatrix = $this->Flights_Model->getFlightSearchResultmatrix_round($session_id,$akbar_session);
        $data['flight_result']=$flightResult['search_result'];
        $flight_result = $data['flight_result'];
        $_SESSION[$rand_id]['flight_result1'] = $flight_result;
        if($data['flight_result']!='')
        ...
        $matrixData['flight_result']=$flight_resultmatrix;
        $matrixData['session_id']=$session_id;
        $matrixData['akbar_session']=$akbar_session;
        $airline_filter_result=$this->load->view('flights/search_result_airline_ajax_round',$airline_data,true);
        $stops_filter_result=$this->load->view('flights/search_result_stops_ajax_round',$stops_data,true);
        $matrix_search_result= $this->load->view('flights/search_result_matrix_round',$matrixData,true);
        echo json_encode(array(
            'flight_search_result' => $flight_search_result,
            'airline_filter_search' => $airline_filter_result,
            'stops_filter_search' => $stops_filter_result,
    ...
    function pro_pre_booking($id,$rand_id,$ref_id='',$err='')
    {
	if($this->input->post('guest_booking'))
        {
            ...
            else
            {
				...
				if ($flight_result->journey_types == "Round") 
				{
					redirect('flights/pro_pre_booking/'.$id.'/'.$rand_id.'/'.$ref_id.'/err_flag/#exist_error', 'refresh');
				} 
				else if ($flight_result->journey_type == "MultiCity") 
				{
					redirect('flights/pro_pre_booking/'.$id.'/'.$rand_id.'/'.$ref_id.'/err_flag/#exist_error', 'refresh');
					...
					redirect('flights/pro_pre_booking/'.$id.'/'.$rand_id.'/--/err_flag/#exist_error', 'refresh');
					//$this->load->view('flights/flight_details', $data);
			 }
		}
        elseif($this->input->post('user_booking') || $this->input->post('user_booking2'))
        {
	...
	function currency_converter()
	{
		...
		$to = "INR";
		$amount = 1;
		$key = 'S6nYK-eNu68-GVJah';
		$url = "http://www.exchangerate-api.com/".$from."/".$to."/".$amount."?k=".$key."";
...
######################################################################################################
                ...
                if($check == 0)
                {
                    $user_title = $_SESSION['post']['user_title'] = $_POST['user_title'];
                    $user_fname = $_SESSION['post']['user_fname'] = $_POST['user_fname'];
                    $user_lname = $_SESSION['post']['user_lname'] = $_POST['user_lname'];
                    $user_address = $_SESSION['post']['user_address'] = $_POST['user_address'];
                    $user_city = $_SESSION['post']['user_city'] = $_POST['user_city'];
                    $user_state = $_SESSION['post']['user_country'] = $_POST['user_country'];
                    $user_country = $_SESSION['post']['user_state'] = $_POST['user_state'];
                    $user_mobile = $_SESSION['post']['user_mobile'] = $_POST['user_mobile'] ;
                    $akbar_ref = "akb".rand(100,100000);
                    $password = "akb".rand(100,100000);
                    $insert_id=$this->Flights_Model->insert_mastercustomer($user_email,$user_title,$user_fname,$user_lname,$user_address,$user_city,$user_state,$user_country,$user_mobile,$akbar_ref,$password);
                    ################### SEND CONFIRM MAIL TO USER STARTS ################################
                    $msg='<table>
                            <tr><td>Dear '.$user_title.' '.$user_fname.'</td><tr>
                                <tr><td>Thank you for booking from Akbartravels.us <br /><br />
                                    You can dirctly login with to our site , with the below credentials :<br />
                                    Email: '.$user_email.'<br />
                                    Password: '.$password.'<br /><br />

                                    Thanks & Regards,<br />
                                    info@akbartravels.us<br />
                                    www.akbartravels.us<br />
                                </td></tr>
                           </table>';

                    $toEmail=$email;
                    $fromName='Akbartravels';
                    $fromEmail='info@akbartravels.us';
                    $ccEmail='jalaj.provab@gmail.com';
                    $subject='Login Details For akbartravels.us';
                    send_mail($user_email,$fromName,$fromEmail,$ccEmail,$msg,$subject);// function called from user helper.
                    ################### SEND CONFIRM MAIL TO USER ENDS ##################################
                    $this->session->set_userdata(array('customer_id' => $insert_id, 'cust_email' => $user_email));

                }         
                
                $email = $_POST["user_email"];
                $cardno  = $this->input->post('card_number');
                
                $user_id=$this->session->userdata('customer_id');
                $this->db->query($sql="insert into centralized_bookings set userid='".$user_id."', utl='".$_SESSION['akbar_session']."',serv_type='Flight',serv_provider='Amadeus',status='Pending',created_on='".date('Y-m-d H:i:s')."'");
                $centralisedId=$this->db->insert_id();
                $booking_id='ATUS'.$centralisedId;
		 //$this->hdfc_payment_gateway();

                $_SESSION['post']['booking_id'] = $booking_id;
                $data['booking_id'] = $_SESSION['post']['booking_id'];
                $data['akbar_session_id'] = $_SESSION['akbar_session'];
               // $this->Flights_Model->insert_booking_id($data);
		#################################################################################
                    ...
                    {
						$depLocArr=explode('<br>',$res['dlocation']);
						$arvLocArr=explode('<br>',$res['alocation']);
						$depLoc=$depLocArr[0];
						$arvLoc=end($arvLocArr);
					}
					else
					{
						$depLoc=$res['dlocation'];
						$arvLoc=$res['alocation'];
					}
					$CardType = $this->input->post('card_type');
                    $departureCountry=$this->Flights_Model->get_airportname($depLoc);
                    $arrartureCountry=$this->Flights_Model->get_airportname($arvLoc);
                    $fopCheck= 'false';
                    if(($departureCountry->country=='USA' && $arrartureCountry->country=='USA') || ($CardType=='AX'))
                    {
						$fopCheck= 'true';
					}
                    $_SESSION['post']['fopcheck']=$fopCheck;
                    
                    if($res['journey_type']=='OneWay')
                    {
                        $db_insert_flightDetails = $res;
                        $db_insert_flightDetails1 = $res;
                        //echo '<pre />';print_r($db_insert_flightDetails);die;
                        if(strpos($db_insert_flightDetails['dep_date'],'<br>'))
                        {
                           $dpDateArr=explode('<br>',$db_insert_flightDetails['dep_date']);
                           $departureDateArr=explode('/',$dpDateArr[0]);
                        }
                        else
                        {
                            $departureDateArr=explode('/',$db_insert_flightDetails['dep_date']);
                        }
                        
                        if($_SESSION['post']['fopcheck']=='true')
                        {
							$gateway_type='CRS';
						}
						else
                        ...
                        $departureDate='20'.$departureDateArr[2].'-'.$departureDateArr[1].'-'.$departureDateArr[0];
                        $totalPrice=$db_insert_flightDetails['Total_FareAmount']+$db_insert_flightDetails['admin_markup']+$db_insert_flightDetails['payment_charge'];
                        $pnr_id = $this->Flights_Model->final_response_details_booking_new($_SESSION['post'],$db_insert_flightDetails['admin_markup'],$db_insert_flightDetails['payment_charge'],$totalPrice,$booking_id,$departureDate,$_SESSION['akbar_session'],$gateway_type);
                        $this->Flights_Model->booking_airline_details($db_insert_flightDetails,$pnr_id);
                        //$this->Flights_Model->bookingpassenger_details($_SESSION['post'],$pnr_id,$flightDetails['Adult_FareAmount'],$res['Adult_TaxAmount'],$res['Child_FareAmount'],$res['Child_TaxAmount'],$res['Infant_FareAmount'],$res['Infant_TaxAmount']);
						$this->Flights_Model->bookingpassenger_details($_SESSION['post'],$pnr_id,$res['Adult_FareAmount'],$res['Adult_TaxAmount'],$res['Child_FareAmount'],$res['Child_TaxAmount'],$res['Infant_FareAmount'],$res['Infant_TaxAmount']);
						
						#############################INVOICE INSERT ####################################
							$book_date = date('Y-m-d');
							
							$this->Flights_Model->invoice_insert($booking_id,$book_date,$res['dlocation'],$res['alocation'],$res['dateOfDeparture'],$_POST['user_title'],$_POST['user_fname'],$_POST['user_lname'],$res['fareType'],$res['cabin'],$res['cicode'],$res['Adult_FareAmount'],$res['Adult_TaxAmount'],$res['Child_FareAmount'],$res['Child_TaxAmount'],$res['Infant_FareAmount'],$res['Infant_TaxAmount'],$gateway_type);
						#############################INVOICE INSERT ####################################
                    }
                    if ($res['journey_type'] == "Round_oneway") 
                    {
                        ...
                        if(strpos($db_insert_flightDetails_oneway['dep_date'],'<br>'))
                        {
                           $dpDateArr=explode('<br>',$db_insert_flightDetails_oneway['dep_date']);
                           $departureDateArr=explode('/',$dpDateArr[0]);
                        }
                        else
                        {
                            $departureDateArr=explode('/',$db_insert_flightDetails_oneway['dep_date']);
                        }
                        $departureDate='20'.$departureDateArr[2].'-'.$departureDateArr[1].'-'.$departureDateArr[0];
                        if($_SESSION['post']['fopcheck']=='true')
                        {
                        ...
                        $totalPrice=($db_insert_flightDetails_oneway['Total_FareAmount']+$db_insert_flightDetails_return['Total_FareAmount'])+$db_insert_flightDetails_oneway['admin_markup']+$db_insert_flightDetails_oneway['payment_charge'];
                        $pnr_id = $this->Flights_Model->final_response_details_booking_new($_SESSION['post'],$db_insert_flightDetails_oneway['admin_markup'],$db_insert_flightDetails_oneway['payment_charge'],$totalPrice,$booking_id,$departureDate,$_SESSION['akbar_session'],$gateway_type);
                        ...
                        $Adult_FareAmount=$db_insert_flightDetails_oneway['Adult_FareAmount']+$db_insert_flightDetails_return['Adult_FareAmount'];
                        $Adult_TaxAmount=$db_insert_flightDetails_oneway['Adult_TaxAmount']+$db_insert_flightDetails_return['Adult_TaxAmount'];
                        $Child_FareAmount=$db_insert_flightDetails_oneway['Child_FareAmount']+$db_insert_flightDetails_return['Child_FareAmount'];
                        $Child_TaxAmount=$db_insert_flightDetails_oneway['Child_TaxAmount']+$db_insert_flightDetails_return['Child_TaxAmount'];
                        $Infant_FareAmount=$db_insert_flightDetails_oneway['Infant_FareAmount']+$db_insert_flightDetails_return['Infant_FareAmount'];
                        $Infant_TaxAmount=$db_insert_flightDetails_oneway['Infant_TaxAmount']+$db_insert_flightDetails_return['Infant_TaxAmount'];
                        
                        $this->Flights_Model->bookingpassenger_details($_SESSION['post'],$pnr_id,$Adult_FareAmount,$Adult_TaxAmount,$Child_FareAmount,$Child_TaxAmount,$Infant_FareAmount,$Infant_TaxAmount,$db_insert_flightDetails_oneway['admin_markup'],$db_insert_flightDetails_oneway['payment_charge'],$totalPrice);
                ...
                #################################################################################         
					$res = $this->Flights_Model->check_ebs_validate($email,$cardno); 
                    if(isset($res) && $res != '')
                    {
                        if($res->status == 'Whitelisted')
                        {
                            ...
                            if($fopCheck=='true')
                            {
								$this->booking_final_payment($_SESSION['get']['id'],$_SESSION['get']['rand_id']);
							}
							else
							{
								$this->hdfc_payment_gateway();
                    ...
                    else 
                    {
                             ...
                             $costval = $this->currency_converter();
                                    //echo $amount; exit;
                             $cardno  = $this->input->post('card_number');
                             $username = md5($this->input->post("user_email"));
                                   
                                    $key = 'd03d461d634dccc424685375e7f84056';
                                    $Extra1 = $this->input->post('user_address');
                                    $Extra2 = $this->input->post('user_address');
                                    $Extra3 = 'Flight Booking';
                                    //$amount =  100;
                                    $ShipCity = $this->input->post('user_city');
                                    $ShipEmail = $this->input->post("user_email");
                                    $CustPhone = $ShipPhone = $this->input->post('user_mobile');
                                    $ShipPostal = '560016';
                                    $Country = 'IN';
                                    ...
                                    $ShipMethod = 'Booking';
                                    $CardNumberHash = md5($cardno);
                                    $CardNumber = $this->input->post('card_number');
                                   
                                    $PassMD5 = md5('123465');
                                    $ShipAddress = $this->input->post('billing_address');
                                    $Address = $this->input->post('billing_address');
                                    $ShipName = $this->input->post('cardfirst_name')."".$this->input->post('cardlast_name');
                                   
                                    $CardType = $this->input->post('card_type');
                                    $ReferenceNo = rand(1,10000);
                                    $IP = '123.201.254.254';
                                    $Postal = '560016';
                                    $ShipCountry = 'IN';
                                    $bin = substr($cardno,0,6); 
                                    $UserMD5 = '76f84f68e691d7954d44d13900413cdb';
                                    $Name = $this->input->post('user_fname')."".$this->input->post('user_lname');

                                    $amount = floor($amount * $costval);
                                    $postal = $this->input->post('user_pincode'); 
                                    //echo $booking_id;
                                    $date = date('Y-m-d H:i:s');
                             $request = "https://s1.rmsid.com/v2/api/txn/Post.xml/?Bin=526701&ReferenceNo=".$booking_id."&CustPhone=".$CustPhone."&UserMD5=".$UserMD5."&Name=".$Name."&Extra1=".$Extra1."&Extra2=".$Extra2."&Amount=".$amount."&ShipCity=".$ShipCity."&Products=AirTickets&ShipEmail=".$this->input->post("user_email")."&PaymentStatus=Y&Country=".$this->input->post('card_country')."&NameOnCard=".$ShipName."&City=".$ShipCity."&ShipPeriod=1&DateTime=".$date."&Region=".$this->input->post('user_state')."&ShipRegion=".$this->input->post('user_state')."&Key=".$key."&ShipMethod=Booking&CardNumberHash=".$CardNumberHash."&CardNumber=".$cardno."&CustomerIsReliable=N&AccountID=1019&Cardsuffix=123&CustomerID=".$username."&ShipPhone=".$ShipPhone."&ShipPostal=".$postal."&PassMD5=3d9188577cc9bfe9291ac66b5cc872b7&Device=".$username."&ShipAddress=".$this->input->post('user_address')."&Address=".$this->input->post('user_address')."&ShipName=".$Name."&SessionID=".$_SESSION['akbar_session']."&CardType=".$CardType."&CustEmail=".$this->input->post("user_email")."&Extra3=FlightBooking&IP=123.201.254.254&TemplateID=66&Postal=".$postal."&ShipCountry=IN";
                            //echo $request;  exit;
                            $xmlinfo = simplexml_load_file($request);
                            //echo '<pre/>';
                            //print_r($xmlinfo);
                            //exit;
                            //$xmlinfo_string = htmlentities($xmlinfo);
                            $myFile = $_SESSION['file_num']."-".$_SESSION['akbar_session']."ebs_request.txt";
                            $fh = fopen('xmllogs/'.$myFile, 'w','r');
                            $stringData = "EBS XML Request : ".$request;
                            fwrite($fh, $stringData);
                            fclose($fh);
                            //print_r($xmlinfo);
                            $xml_infstring = $xmlinfo->asXML();			
                            //echo '<pre/>';
                            //print_r($xml_infstring);
                            $myFile1 = $_SESSION['file_num']."-".$_SESSION['akbar_session']."ebs_response.txt";
                            $fh1 = fopen('xmllogs/'.$myFile1, 'w','r');
                            //$xml->asXML($xmlinfo);
                            $stringData = "EBS XML Response : ".$xml_infstring;
                            fwrite($fh1, $stringData);
                            fclose($fh1);
                            ...
                            $method = 'EBS Authentication XML';
                            $this->Flights_Model->insert_logs_security($_SESSION['akbar_session'],$method,$myFile,$myFile1,$_SESSION['journey_type']);
                            //exit;
                            if(isset($xmlinfo->ReferenceNo))
                            {
                                    $ReferenceNo = $xmlinfo->ReferenceNo;
                                    $TxnLogID = $xmlinfo->TxnLogID;
                                    $CreatedOn = $xmlinfo->CreatedOn;
                                    $RiskLevel = $xmlinfo->RiskLevel;
                                    $RiskPercentage = $xmlinfo->RiskPercentage;
                                    $Amount = $xmlinfo->Amount;
                                    $Output = $xmlinfo->Output;
                                    $AccountId = $Output->AccountId;
                                    $IpDistance = $Output->IpDistance;
                                    $IpCity = $Output->IpCity;
                                    $IpRegion = $Output->IpRegion;
                                    ...
                                    $this->Flights_Model->insert_ebs($ReferenceNo,$TxnLogID,$CreatedOn,$RiskLevel,$RiskPercentage,$email,$status,$CardNumber,$card_cvv,$exp_month,$exp_year);
                                   $_SESSION['post']['booking_id'] = $booking_id;
									if($fopCheck=='true')
									{
										$this->booking_final_payment($_SESSION['get']['id'],$_SESSION['get']['rand_id']);
                                    ...
                                    {
                                            redirect('flights/pre_booking/'.$id."/".$rand_id,'refresh');
                                    }*/
                            }
                            else
                            {
	...
	/***** payment Gateway ******/
    function hdfc_payment_gateway()
    {
		$data['entered_date'] = $_SESSION['pay_date'] = $date = date('d-M-Y');
		$data['time'] = $_SESSION['pay_time'] = $time = date('g:i:s');
		$data['mtrackid'] = $_SESSION['pay_mtrackid'] = $mtrackid=time();
		$data['ip'] = $_SESSION['pay_ip'] = $IP = $_SERVER['REMOTE_ADDR'];
		$data['source'] = $_SESSION['pay_source'] = $source = 'Flight Ticket';//
		
		$email = $_SESSION['post']['user_email'];
		$card_number = $_SESSION['post']['card_number'];
		$amount = $_SESSION['post']['amount'];
		$_SESSION['post']['cardfirst_name'] = $_POST['cardfirst_name'];
		$_SESSION['post']['cardlast_name'] = $_POST['cardlast_name'];
		
		
		$data['amount'] = $_SESSION['pay_amt'] = $AMT = $amount; //
		$data['email_id'] =$_SESSION['pay_email'] = $email;//
		$data['session_email'] = $_SESSION['pay_user_id'] = $user_id = $email;//
		$data['name'] = $name = $_SESSION['post']['cardfirst_name']."".$_SESSION['post']['cardlast_name'];//
		$data['mobile'] =$_SESSION['pay_mobile'] = $mobile = $_SESSION['post']['user_mobile'];//
		$data['booking_id'] =$_SESSION['pay_booking_id'] = $booking_id = $_SESSION['post']['booking_id'];//
		$data['status'] =$_SESSION['pay_status'] = $status = 'PENDING';
		$data['trackid_status'] =$_SESSION['trackid_status'] = $trackid_status = 'Yes';
		
		$data['cardno'] = $_SESSION['pay_cardno'] = $cardno = $_SESSION['post']['card_number'];//
		$data['cvv'] = $_SESSION['pay_cvv'] = $cvv =  $_SESSION['post']['card_cvv'];//
		$data['expmm'] =$_SESSION['pay_expmm'] = $expmm =  $_SESSION['post']['exp_month'];//
		$data['expyy'] =$_SESSION['pay_expyy'] = $expyy =  '2013';//"20".$_SESSION['post']['exp_year'];//
		$data['membername'] =$_SESSION['pay_membername'] = $membername = $_SESSION['post']['cardfirst_name']."".$_SESSION['post']['cardlast_name'];//card name
		
		//echo "<pre/>"; print_r($data);exit;
		$insert_id = $this->Flights_Model->hdfc_payment_gateway($data,$_SESSION['post']['booking_id']);
		$this->load->view('pay',$data);
	}
	
    function send_perform_request()
    {
            $data['mtrackid'] = $_SESSION['pay_mtrackid'];
            $this->load->view('TranVbyV',$data);	
    }

    function status_transaction()
    {
            //echo "<pre/>";print_r($_REQUEST);
            if(isset($_REQUEST['ResError']))
            {
                    $data['pg_response'] = $ResError = $_REQUEST['ResError'];
                    $data['mtrackid'] = $ResTrackId = $_REQUEST['ResTrackId'];
                    $data['trackid_status'] = $trackid_status = 'No';
                    $data['status'] = 'Failed';
                    $this->Flights_Model->update_payment_gateway_status($data);
                    //go to error page
            ...
            else
            {
                    if(count($_REQUEST)==6)
                    {
                            ...
                            $data['pg_response'] = $ResResult = $_REQUEST['ResResult'];
                            $data['mtrackid'] = $ResTrackId = $_REQUEST['ResTrackId'];
                            $data['PaymentID'] = $ResPaymentId = $_REQUEST['ResPaymentId'];
                            $data['res_ref'] = $ResRef = $_REQUEST['ResRef'];
                            $data['TransactionID'] = $ResTranId = $_REQUEST['ResTranId'];
                            $data['amount'] = $ResAmount = $_REQUEST['ResAmount'];
                            $data['status'] = 'Success';
                            $data['trackid_status'] = $trackid_status = 'No';
                            $this->Flights_Model->update_payment_gateway_status($data);
                            //success page
                    ...
                    else
                    {
                            ...
                            $data['pg_response'] = $ResResult = $_REQUEST['ResResult'];
                            $data['mtrackid'] = $ResTrackId = $_REQUEST['ResTrackId'];
                            $data['PaymentID'] = $ResRef = $_REQUEST['ResPaymentId'];
                            $this->Flights_Model->update_payment_gateway_status($data);
                            //$msg = "Error in Transaction";
                            //print_r($_REQUEST);exit;
                            $this->error_page($ResResult);
                            //error page
                    }
    ...
    /***** End Payment Gateway ******/
    function booking_final_payment($id, $rand_id)
    {	
            $user_email = $_SESSION['post']['user_mobile'];
            ...
            $user_city = $_SESSION['post']['user_city'];
            $user_state = $_SESSION['post']['user_country'];
            $user_country = $_SESSION['post']['user_state'];
...
##########################################################################################################
        ...
        else if ($res['journey_type'] == "MultiCity")
        {
        ...
        else if ($res['journey_type'] == "OneWay") 
        {
            $flightDetails = $res;
		...
		############################################ INSERTING LOGS #######################################################################
		$myFile = $_SESSION['file_num']."-".$_SESSION['akbar_session']."Air_SellFromRecommendation_request.txt";
		$fh = fopen('xmllogs/'.$myFile, 'w','r');
		$stringData = "Booking XML Request".$SellFromRecommendation;
		fwrite($fh, $stringData);
		fclose($fh);

		$myFile1 = $_SESSION['file_num']."-".$_SESSION['akbar_session']."Air_SellFromRecommendationReply.txt";
		$fh1 = fopen('xmllogs/'.$myFile1, 'w','r');
		$stringData = "Booking XML Response ".$dataSellFromRecommendation;
		fwrite($fh1, $stringData);
		fclose($fh1);
		$method = 'Booking XML';
		$this->Flights_Model->insert_logs_security($_SESSION['akbar_session'],$method,$myFile,$myFile1,$res['journey_type']);
		############################################ INSERTING LOGS#######################################################################
        if (!empty($dataSellFromRecommendation)) 
        {
            ...
            //$status_flag = "false";
            if (($res['journey_type'] == "Round_oneway") || ($res['journey_type'] == "MultiCity")) 
            {
            ...
            {		
                    ...
                    for ($ad = 0; $ad < $adult; $ad++) {
                        $travellerInformation_ADT.='<passengerData>
                                                            <travellerInformation>
                                                                    <traveller>
                                                                            <surname>' . ($_SESSION['post']['fnameadult'][$ad]) . '</surname>
                                                                            <quantity>1</quantity>
                                                                    </traveller>
                                                                    <passenger>
                                                                            <firstName>' . ($_SESSION['post']['lnameadult'][$ad]) . '</firstName>
                                                                    </passenger>
                                                            </travellerInformation>
                                                    </passengerData>';
                ...
                else 
                {
                    ...
                    {
                        $result_dob_inf = str_replace("-", "", $_SESSION['post']['adobinfant'][$ad]);
                        $travellerInformation_ADT.='<passengerData>
                                                        <travellerInformation>
                                                                <traveller>
                                                                        <surname>' . ($_SESSION['post']['fnameadult'][$ad]) . '</surname>
                                                ...
                                                <passengerData> 
                                                        <travellerInformation> 
                                                                <traveller> 
                                                                        <surname>' . ($_SESSION['post']['fnameinfant'][$ad]) . '</surname> 
                                                                </traveller> 
                                                                <passenger>
                                                                        <firstName>' . ($_SESSION['post']['lnameinfant'][$ad]) . '</firstName>
                                                                        <type>INF</type>
                                                                </passenger>
                                                        </travellerInformation> 
                                                        <dateOfBirth> 
                    ...
                    if ($adult > $infant) 
                    {
                        ...
                        for ($ad1 = $infant; $ad1 < $adult; $ad1++) 
                        {
                            $travellerInformation_ADT.='<passengerData>
                                                                <travellerInformation>
                                                                        <traveller>
                                                                                <surname>' . ($_SESSION['post']['fnameadult'][$ad1]) . '</surname>
                                                                                <quantity>1</quantity>
                                                                        </traveller>
                                                                        <passenger>
                                                                                <firstName>' . ($_SESSION['post']['lnameadult'][$ad1]) . '</firstName>
                                                                        </passenger>
                                                                </travellerInformation>
                                                        </passengerData>';
                ...
                if (!empty($child)) {
                    for ($ch = 0; $ch < $child; $ch++) {
                        $result_dob = str_replace("-", "", $_SESSION['post']['adobchild'][$ch]);
                        $travellerInformation_CHD.='<passengerData>
                                                            <travellerInformation>
                                                                    <traveller>
                                                                            <surname>' . ($_SESSION['post']['fnamechild'][$ch]) . '</surname>
                                                                            <quantity>1</quantity>
                                                                    </traveller>
                                                                    <passenger>
                                                                            <firstName>' . ($_SESSION['post']['lnamechild'][$ch]) . '</firstName>
                                                                            <type>CHD</type>
                                                                    </passenger>
                                                            </travellerInformation>
                                                            <dateOfBirth>
                ...
                if (isset($_SESSION['post']['user_country'])) 
                {
                    if (!empty($_SESSION['post']['user_country'])) 
                    {
                        //$code = $this->Flights_Model->get_countyr_code($value['country']);
                        //$countyr_code_final = $code[0]->iso3;
                        $countyr_code_final = $_SESSION['post']['user_country'];
                    }
                    else
                        $countyr_code_final = '';
                }
                ...
                for ($i = 0; $i < count($_SESSION['post']['saladult']); $i++) 
                {
                    if ($_SESSION['post']['saladult'][$i] == "Mr")
                        $gender[$i] = "M";
                    else
                        $gender[$i] = "F";

                    if (isset($_SESSION['post']['passportadult'])) 
                    {
                        if (!empty($_SESSION['post']['passportadult'][$i]))
                            $passport_no[$i] = $_SESSION['post']['passportadult'][$i];
                        else
                            $passport_no[$i] = '';
                    ...
                    if (isset($_SESSION['post']['visaadult'])) 
                    {
                        if (!empty($_SESSION['post']['visaadult'][$i])) {
                            $visa_no = $_SESSION['post']['visaadult'][$i];
                        } else {
                            $visa_no = '';
                        }
                    }
                    ...
                    if (isset($_SESSION['post']['Pcountry'])) 
                    {
                        if (!empty($_SESSION['post']['Pcountry'][$i])) 
                        {
                            $code = $this->Flights_Model->get_countyr_code($_SESSION['post']['Pcountry'][$i]);
                    ...
                    if (isset($_SESSION['post']['plname'])) 
                    {
                        if (!empty($_SESSION['post']['plname'][$i])) {
                            $plname = $_SESSION['post']['plname'][$i];
                        } else {
                            $plname = '';
                        }
                    }
                    ...
                    if (isset($_SESSION['post']['pexpiry'][$i])) {
                        if (!empty($_SESSION['post']['pexpiry'][$i])) {
                    ...
                    $fullname_adult = $_SESSION['post']['fnameadult'][$i] . "/" . $_SESSION['post']['lnameadult'][$i] . " " . $plname;
                    if(isset($_SESSION['post']['adobadult']))
                    {
                        $dadult = $_SESSION['post']['adobadult'][$i];
                ...
                $j = $i;
                if (isset($_SESSION['post']['salchild'])) 
                {
                    for ($i = 0; $i < count($_SESSION['post']['salchild']); $i++) 
                    {
                        if ($_SESSION['post']['salchild'][$i] == "Mr")
                            $gender[$i] = "M";
                        else
                            $gender[$i] = "F";


                        if (isset($_SESSION['post']['passportchild'])) 
                        {
                            if (!empty($_SESSION['post']['passportchild'][$i])) {
                                $passport_no_child[$i] = $_SESSION['post']['passportchild'][$i];
                            } else {
                                $passport_no_child[$i] = '';
                            }
                        }
                        ...
                        if (isset($_SESSION['post']['visachild'])) 
                        {
                            if (!empty($_SESSION['post']['visachild'][$i])) {
                                $visa_no_child = $_SESSION['post']['visachild'][$i];
                            } else {
                                $visa_no_child = '';
                            }
                        }
                        ...
                        if (isset($_SESSION['post']['Pcountrychild'])) 
                        {
                            if (!empty($_SESSION['post']['Pcountrychild'][$i])) {
                                $code = $this->Flights_Model->get_countyr_code($_SESSION['post']['Pcountrychild'][$i]);
                        ...
                        if (isset($_SESSION['post']['pexpirychild'][$i])) 
                        {
                            if (!empty($_SESSION['post']['pexpirychild'][$i])) {
                                $dchild_exp = $_SESSION['post']['pexpirychild'][$i];
                        ...
                        if (isset($_SESSION['post']['plnamechild'])) 
                        {
                            if (!empty($_SESSION['post']['plnamechild'][$i])) {
                                $plnamechild = $_SESSION['post']['plnamechild'][$i];
                            } else {
                                $plnamechild = '';
                            }
                        }
                        ...
                        $fullname_child = $_SESSION['post']['fnamechild'][$i] . "/" . $_SESSION['post']['lnamechild'][$i] . " " . $plnamechild;
                        $dchild = $_SESSION['post']['adobchild'][$i];
                ...
                if (isset($_SESSION['post']['salinfant'])) 
                {
                    for ($i = 0; $i < count($_SESSION['post']['salinfant']); $i++) 
                    {
                        if ($_SESSION['post']['salinfant'][$i] == "Mr")
                            $gender[$i] = "MI";
                        else
                            $gender[$i] = "FI";

                        if (isset($_SESSION['post']['passportinfant'])) 
                        {
                            if (!empty($_SESSION['post']['passportinfant'][$i]))
                                $passport_no_child[$i] = $_SESSION['post']['passportinfant'][$i];
                            else
                                $passport_no_child[$i] = '';
                        }
                        else $passport_no_child[$i] = '';
                        
                        if (isset($_SESSION['post']['visainfant'])) 
                        {
                            if (!empty($_SESSION['post']['visainfant'][$i]))
                                $visa_no_child = $_SESSION['post']['visainfant'][$i];
                            else
                                $visa_no_child = '';
                        }
                        else $visa_no_child = '';
                        
                        if (isset($_SESSION['post']['Pcountryinfant'])) 
                        {
                            if (!empty($_SESSION['post']['Pcountryinfant'][$i])) {
                                $code = $this->Flights_Model->get_countyr_code($_SESSION['post']['Pcountryinfant'][$i]);
                                $countyr_code_child[$i] = $code[0]->iso3;
                            }
                            else
                                $countyr_code_child[$i] = '';
                        ...
                        if (isset($_SESSION['post']['pexpiryinfant'][$i])) 
                        {
                            if (!empty($_SESSION['post']['pexpiryinfant'][$i])) {
                                $dinfant_exp = $_SESSION['post']['pexpiryinfant'][$i];
                        ...
                        if (isset($_SESSION['post']['plnameinfant'])) 
                        {
                            if (!empty($_SESSION['post']['plnameinfant'][$i]))
                                $plnameinfant = $_SESSION['post']['plnameinfant'][$i];
                            else
                                $plnameinfant = '';
                        }
                        else $plnameinfant = '';
                        
                        $fullname_infant = $_SESSION['post']['fnameinfant'][$i] . "/" . $_SESSION['post']['lnameinfant'][$i] . " " . $plnameinfant;
                        $dinfant = $_SESSION['post']['adobinfant'][$i];
                ...
                {
                    $dlocArray=explode('<br>',$flightDetails_oneway['dlocation']);
                    $alocArray=explode('<br>',$flightDetails_oneway['alocation']);
                }
                $dloc=$dlocArray[0];
                $aloc=end($alocArray);
                ...
                $fop='';
                if($internationalCheck=='false' || $_SESSION['post']['card_type']=='AX')
                {
                    if($res['journey_type']=='OneWay')
                    {
                        $cardText1='CardType-CC/TotalAmount-'.$flightDetails["Total_FareAmount"].'/CardName-AX/CardNumber-'.$_SESSION['post']['card_number'].'/CardExpiryDate-'.$_SESSION['post']['exp_year'].$_SESSION['post']['exp_month'];
                    }
                    else
                    {
                        $cardText1='CardType-CC/TotalAmount-'.($flightDetails_oneway["Total_FareAmount"]+$flightDetails_return["Total_FareAmount"]).'/CardName-AX/CardNumber-'.$_SESSION['post']['card_number'].'/CardExpiryDate-'.$_SESSION['post']['exp_year'].$_SESSION['post']['exp_month'];
                    }
                    $cardText='<dataElementsIndiv>
										<elementManagementData>
				...
				$booking_id = $_SESSION['post']['booking_id'];
                if($res['journey_type']=='OneWay')
                {
                    $baseFare=($flightDetails["Total_FareAmount"]-$flightDetails["TaxAmount"])-$flightDetails["admin_markup"];
                    $RM_freeText='AIAN/2011007775/'.$booking_id.'/'.$flightDetails["Total_FareAmount"].'/'.$flightDetails["TaxAmount"].'/'.$flightDetails["admin_markup"];
                }
                else
                {
                    $baseFare=(($flightDetails_oneway["Total_FareAmount"]+$flightDetails_return["Total_FareAmount"])-($flightDetails_oneway["TaxAmount"]+$flightDetails_return["TaxAmount"]))-$flightDetails_oneway["admin_markup"];
                    $RM_freeText='AIAN/2011007775/'.$booking_id.'/'.($flightDetails_oneway["Total_FareAmount"]+$flightDetails_return["Total_FareAmount"]).'/'.($flightDetails_oneway["TaxAmount"]+$flightDetails_return["TaxAmount"]).'/'.$flightDetails_oneway["admin_markup"];
                }
                
                $fop='';
                if($_SESSION['post']['fopcheck']=='true')
                {
					$fop.='<dataElementsIndiv>
									<elementManagementData>
				...
				else if($_SESSION['post']['fopcheck']=='false')
				{
						$fop.='<dataElementsIndiv>
									<elementManagementData>
											<segmentName>FP</segmentName>
                                                            ...
                                                            <dataElementsMaster>
                                                                    ...
                                                                    <dataElementsIndiv>
                                                                            ...
                                                                            <freetextData>
                                                                                    ...
                                                                                    <longFreetext>' . $_SESSION['post']['user_mobile'] . '</longFreetext>
                                                                            </freetextData>
                                                                    </dataElementsIndiv>
                                                                    <dataElementsIndiv>
                                                                            <elementManagementData>
                ...
                //echo "PNR_AddMultiElements Request:<br/>".$PNR_AddMultiElements."<br />";//die;
                //$URL2 = "https://test.webservices.amadeus.com";
                $URL2 = "https://production.webservices.amadeus.com";
                ...
                // Execute request, store response and HTTP response code
                $PNRAddMultiElementsRes = curl_exec($ch2);
                $error2 = curl_getinfo($ch2, CURLINFO_HTTP_CODE);
                curl_close($ch2);
                //echo "PNRAddMultiElements Response :<br/>";print_r($PNRAddMultiElements);die;
                $PNRAddMultiElements_result = $this->xml2array($PNRAddMultiElementsRes);
               //echo'<pre/>: Paasenger Information';print_r($PNRAddMultiElements_result);exit;
				############################################ INSERTING LOGS #######################################################################
				$myFile = $_SESSION['file_num']."-".$_SESSION['akbar_session']."PNR_AddMultiElements_Request.txt";
				$fh = fopen('xmllogs/'.$myFile, 'w','r');
				$stringData = "Booking XML Request".$PNR_AddMultiElements;
				fwrite($fh, $stringData);
				fclose($fh);

				$myFile1 = $_SESSION['file_num']."-".$_SESSION['akbar_session']."PNRAddMultiElements_Response.txt";
				$fh1 = fopen('xmllogs/'.$myFile1, 'w','r');
				$stringData = "Booking XML Response ".$PNRAddMultiElementsRes;
				fwrite($fh1, $stringData);
				fclose($fh1);
				$method = 'Booking XML';
				$this->Flights_Model->insert_logs_security($_SESSION['akbar_session'],$method,$myFile,$myFile1,$res['journey_type']);
                ...
                if (!empty($PNRAddMultiElements_result))
                {
                    ...
                    if (!empty($data['PNR_result']))
                    {
                        ...
                        if (true) 
                        {
                            ...
                            if($res['journey_type'] == "Round_oneway")
                                $multTicket=$flightDetails_oneway['MultiTicket'];
                            else if($res['journey_type'] == "OneWay")
                                $multTicket=$flightDetails['MultiTicket'];
                            ...
                            {				
                                $Fare_PricePNRWithBookingClass = '<?xml version="1.0" encoding="utf-8"?>
                                 ...
                                 //echo'Fare_PricePNRWithBookingClass Request111:<br/>'. $Fare_PricePNRWithBookingClass;
                                //$URL2 = "https://test.webservices.amadeus.com";
                                $URL2 = "https://production.webservices.amadeus.com";
                                ...
                                // Execute request, store response and HTTP response code
                                $PricePNRWithBookingClassRes = curl_exec($ch2);
                                $error2 = curl_getinfo($ch2, CURLINFO_HTTP_CODE);
                                curl_close($ch2);
								############################################ INSERTING LOGS #######################################################################
								$myFile = $_SESSION['file_num']."-".$_SESSION['akbar_session']."Fare_PricePNRWithBookingClass_request.txt";
								$fh = fopen('xmllogs/'.$myFile, 'w','r');
								$stringData = "Booking XML Request".$Fare_PricePNRWithBookingClass;
								fwrite($fh, $stringData);
								fclose($fh);

								$myFile1 = $_SESSION['file_num']."-".$_SESSION['akbar_session']."Fare_PricePNRWithBookingClass_Response.txt";
								$fh1 = fopen('xmllogs/'.$myFile1, 'w','r');
								$stringData = "Booking XML Response ".$PricePNRWithBookingClassRes;
								fwrite($fh1, $stringData);
								fclose($fh1);
								$method = 'Booking XML';
								$this->Flights_Model->insert_logs_security($_SESSION['akbar_session'],$method,$myFile,$myFile1,$res['journey_type']);
                                ...
                                $PricePNRWithBookingClass_result = $this->xml2array($PricePNRWithBookingClassRes);
                                ...
                                if (!empty($PricePNRWithBookingClass_result)) 
                                {
                                    ...
                                    else 
                                    {
                                        ...
                                        // Execute request, store response and HTTP response code
                                        $Ticket_CreateTSTFromPricingRes = curl_exec($ch2);
                                        $error2 = curl_getinfo($ch2, CURLINFO_HTTP_CODE);
                                        curl_close($ch2);
                                        //echo "Ticket_CreateTSTFromPricing Response: <br/>";print_r($Ticket_CreateTSTFromPricing);exit;
										############################################ INSERTING LOGS #######################################################################
										$myFile = $_SESSION['file_num']."-".$_SESSION['akbar_session']."Ticket_CreateTSTFromPricing_request.txt";
										$fh = fopen('xmllogs/'.$myFile, 'w','r');
										$stringData = "Booking XML Request".$Ticket_CreateTSTFromPricing;
										fwrite($fh, $stringData);
										fclose($fh);

										$myFile1 = $_SESSION['file_num']."-".$_SESSION['akbar_session']."Ticket_CreateTSTFromPricing_Response.txt";
										$fh1 = fopen('xmllogs/'.$myFile1, 'w','r');
										$stringData = "Booking XML Response ".$Ticket_CreateTSTFromPricingRes;
										fwrite($fh1, $stringData);
										fclose($fh1);
										$method = 'Booking XML';
										$this->Flights_Model->insert_logs_security($_SESSION['akbar_session'],$method,$myFile,$myFile1,$res['journey_type']);
										############################################ INSERTING LOGS#######################################################################	
                                        if (!empty($Ticket_CreateTSTFromPricingRes)) {
                                            $Ticket_CreateTSTFromPricing_result = $this->xml2array($Ticket_CreateTSTFromPricingRes);
                                            ...
                                            if (true) 
                                            {
												...
												$myFile1 = $_SESSION['file_num']."-".$_SESSION['akbar_session']."Fare_PricePNRWithBookingClass_Response.txt";
												$fh1 = fopen('xmllogs/'.$myFile1, 'w','r');
												$stringData = "Booking XML Response ".$PricePNRWithBookingClass;
												fwrite($fh1, $stringData);
												fclose($fh1);
												$method = 'Booking XML';
												$this->Flights_Model->insert_logs_security($_SESSION['akbar_session'],$method,$myFile,$myFile1,$res['journey_type']);
                                            ...
                                            $PNR_AddMultiElements_EndRes = curl_exec($ch2);
                                            $error2 = curl_getinfo($ch2, CURLINFO_HTTP_CODE);
                                            curl_close($ch2);
                                            ############################################ INSERTING LOGS #######################################################################
											$myFile = $_SESSION['file_num']."-".$_SESSION['akbar_session']."PNR_AddMultiElements_End_request.txt";
											$fh = fopen('xmllogs/'.$myFile, 'w','r');
											$stringData = "Booking XML Request".$PNR_AddMultiElements_End;
											fwrite($fh, $stringData);
											fclose($fh);

											$myFile1 = $_SESSION['file_num']."-".$_SESSION['akbar_session']."PNR_AddMultiElements_End_Response.txt";
											$fh1 = fopen('xmllogs/'.$myFile1, 'w','r');
											$stringData = "Booking XML Response ".$PNR_AddMultiElements_EndRes;
											fwrite($fh1, $stringData);
											fclose($fh1);
											$method = 'Booking XML';
											$this->Flights_Model->insert_logs_security($_SESSION['akbar_session'],$method,$myFile,$myFile1,$res['journey_type']);
											############################################ INSERTING LOGS#######################################################################	
                                            $PNR_AddMultiElements_End_result = $this->xml2array($PNR_AddMultiElements_EndRes);
                                            ...
                                            if ($flag_add == true)
                                            {
                                                ...
                                                curl_close($ch2);
												 ############################################ INSERTING LOGS #######################################################################
												$myFile = $_SESSION['file_num']."-".$_SESSION['akbar_session']."PNR_AddMultiElements_End_request2.txt";
												$fh = fopen('xmllogs/'.$myFile, 'w','r');
												$stringData = "Booking XML Request".$PNR_AddMultiElements_End;
												fwrite($fh, $stringData);
												fclose($fh);

												$myFile1 = $_SESSION['file_num']."-".$_SESSION['akbar_session']."PNR_AddMultiElements_End_Response2.txt";
												$fh1 = fopen('xmllogs/'.$myFile1, 'w','r');
												$stringData = "Booking XML Response ".$PNR_AddMultiElements_EndRes;
												fwrite($fh1, $stringData);
												fclose($fh1);
                                            ...
                                            if (!empty($controlNumber)) {
                                                ...
                                                // Execute request, store response and HTTP response code
                                                $PNR_RetrieveRes = curl_exec($ch2);
                                                $error2 = curl_getinfo($ch2, CURLINFO_HTTP_CODE);
                                                curl_close($ch2);
                                                //echo "PNR_AddMultiElements_End Response: <br/>";print_r($PNR_AddMultiElements_End);
                                                ############################################ INSERTING LOGS #######################################################################
												$myFile = $_SESSION['file_num']."-".$_SESSION['akbar_session']."PNR_Retrieve_request.txt";
												$fh = fopen('xmllogs/'.$myFile, 'w','r');
												$stringData = "Booking XML Request".$PNR_Retrieve;
												fwrite($fh, $stringData);
												fclose($fh);

												$myFile1 = $_SESSION['file_num']."-".$_SESSION['akbar_session']."PNR_Retrieve_Response.txt";
												$fh1 = fopen('xmllogs/'.$myFile1, 'w','r');
												$stringData = "Booking XML Response ".$PNR_RetrieveRes;
												fwrite($fh1, $stringData);
												fclose($fh1);
												$method = 'Booking XML';
												$this->Flights_Model->insert_logs_security($_SESSION['akbar_session'],$method,$myFile,$myFile1,$res['journey_type']);
                                                ...
                                                $PNR_Retrieve_result = $this->xml2array($PNR_RetrieveRes);
                                    ...
                                    else 
                                    {
                                        ...
                                        if (!empty($Ticket_CreateTSTFromPricingRes)) 
                                        {
                                           ...
                                           // $URL2 = "https://test.webservices.amadeus.com";
                                            ...
                                            $PNR_AddMultiElements_End_result = $this->xml2array($PNR_AddMultiElements_EndRes);
                                            //echo '<pre/>: PNR_AddMultiElements_End_first:<br> ';print_r($PNR_AddMultiElements_End_result);exit;
                                            $flag_add = false;
                                            $controlNumber = '';
                                            $companyId = '';
                                            if (!empty($PNR_AddMultiElements_End_result)) {
                                            ...
                                            if ($flag_add == true) 
                                            {
                                                ...
                                                $error2 = curl_getinfo($ch2, CURLINFO_HTTP_CODE);
                                                curl_close($ch2);
                                                //echo "PNR_AddMultiElements_End Response1221: <br/>";print_r($PNR_AddMultiElements_End);exit;
												############################################ INSERTING LOGS #######################################################################
												$myFile = $_SESSION['file_num']."-".$_SESSION['akbar_session']."PNR_AddMultiElements_End_request2.txt";
												$fh = fopen('xmllogs/'.$myFile, 'w','r');
												$stringData = "Booking XML Request".$PNR_AddMultiElements_End;
                                            ...
                                            if (!empty($controlNumber)) 
                                            {
                                               ...
                                               // $URL2 = "https://test.webservices.amadeus.com";
                                                ...
                                                if (!empty($PNR_Retrieve_result)) {
                                                    ...
                                                    //echo "dfbshdg<pre/>";print_r($data['PNR_result_final']);echo 'dddd';
                                                    if (!empty($data['PNR_result_final'])) 
                                                    {
                                                        // echo 'asdsa';die;
                                                        $final_result['originatorId'] = $data['PNR_result_final']['sbrPOSDetails']['sbrUserIdentificationOwn']['originIdentification']['originatorId'];
                                                ...
                                                //echo '<pre/>Final Result: ';print_r($final_result);//die;
                                                ...
                                                if ($res['journey_type'] == "Round_oneway") 
                                                {
                                                    ...
                                                    $db_insert_value = $value;
                                                    $db_insert_segmentNameDetails = $data['final_result']['segmentNameDetails'];
                                                    $pnr_id = $this->Flights_Model->final_response_details_booking_update($final_result,$_SESSION['post']['booking_id']);
                                                    //redirect('flights/thankyou/'.$pnr_id,'refresh');
                                                    header('Location:'.site_url().'/flights/thankyou/'.$pnr_id);
                                                } 
                                                else if ($res['journey_type'] == "MultiCity") 
                                                {
                                                ...
                                                else 
                                                {
                                                    ...
                                                    $pnr_id=$this->Flights_Model->final_response_details_booking_update($final_result,$_SESSION['post']['booking_id']);
                                                    header('Location:'.site_url().'/flights/thankyou/'.$pnr_id);
                                                   // redirect('flights/thankyou/'.$pnr_id);
                                                }
...
//echo $url; die;
                    ...
                    foreach($HotelSummary as $hotels)
                    {
                       ...
                       if($hotels->getElementsByTagName('RoomRateDetailsList')->length > 0 && $hotels->getElementsByTagName('RoomRateDetails')->length >0)
                       {
                           ...
                           foreach($roomDetails as $details)
                           {
                               ...
                               $propertyAvailable=$details->getEHotelListlementsByTagName('propertyAvailable')->item(0)->nodeValue;
                               ...
                               $tempData[]=array(
                                   ...
                                   'confidenceRating'=>$confidencHotelListeRating,
    ...
    function get_promo_values()
    {
		$data['promo_code'] = $promo_code = $this->input->post('promo_code');
		...
		//echo $return_date;
		$code_val = $this->Expedia_Model->check_promo_code($promo_code,$total_amount,$departure_date,$return_date);
		$promo = $data['promo_code'];
		//$total_fare= $this->load->view('flights/search_result_airline_ajax',$promo,true);
        if($code_val != '')
		{
			$valid_From  = $code_val->valid_From;
			$valid_to = $code_val->valid_to;
			$discount_type = $code_val->discount_type;
			$discount_mode  = $code_val->discount_mode;
			$max_amount = $code_val->max_amount;
			$value = $code_val->value;
			$sector = $code_val->sector;
			$airline = $code_val->airline;
			$type = $code_val->type;
			$status = $code_val->status;
			if($discount_type == 'Base Fare')
			{
				if($discount_mode == 'Amount')
				{
					$base_fare = $base_fare - $value;
					$total_amount = $tax + $base_fare;
				}
				else
				{
					$base_fare = $base_fare - ($base_fare*$value/100);
					$total_amount = $tax + $base_fare;
				}
			}
			if($discount_type == 'Tax')
			{
				if($discount_mode == 'Amount')
				{
					$tax = $tax - $value;
