...
{
    var flights = $('.resultFlights .searchflight_box').get();
    flights.sort(function(a,b)
    {
		if($sortBy=="data-airline")
		{
		...
		else
		{
			...
			var keyA = parseFloat($(a).find('.FlightInfoBox').attr($sortBy));
			var keyB = parseFloat($(b).find('.FlightInfoBox').attr($sortBy));
		}
		if($order=="asc")
		{
